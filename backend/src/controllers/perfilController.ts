...
export async function getMeuPerfil(req: AuthRequest, res: Response) {
  try {
...
export async function salvarTrajeto(req: AuthRequest, res: Response) {
  const { partida, destino, frequente } = req.body;
  if (!partida || !destino) return res.status(400).json({ erro: 'partida e destino obrigatorios.' });
  try {
...
export async function listarTrajetos(req: AuthRequest, res: Response) {
  try {
